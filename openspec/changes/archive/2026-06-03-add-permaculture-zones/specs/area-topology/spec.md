...
### Requirement: Visualização da Estrutura de Áreas
O sistema SHALL exibir a estrutura de áreas em layout de 3 colunas (Glebas → Talhões → Unidades) com painel de detalhes, substituindo a Tree View aninhada anterior. GG

#### Scenario: Layout anterior removido
- **WHEN** implementação é deployada
- **THEN** a antiga visualização em árvore aninhada com expansão/recolhão é substituída por layout de colunas com seleção
